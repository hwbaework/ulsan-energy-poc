...
function ChartWrapper({ title, description, children, height = 300, className }: ChartWrapperProps) {
  return (
    <div className={cn('h-full rounded-xl bg-[#0d1520] p-5', className)}>
      {(title || description) && (
        <div className="-mx-5 -mt-5 px-5 py-3 mb-4 border-b border-white/[0.06]">
...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: TooltipEntry[]; label?: string }) {
  ...
  return (
    <div
      className="rounded-lg bg-[#0d1520] ring-1 ring-white/10 px-3 py-2 shadow-xl text-xs transition-all duration-300 ease-out"
      style={{
        opacity: visible ? 1 : 0,
...
interface RmsPieChartProps {
  ...
  className?: string;
  donut?: boolean;
  innerRadius?: string; // 기본 도넛 안쪽 (예: '40%')
  outerRadius?: string; // 기본 도넛 바깥 (예: '75%')
}

// 도넛 차트 안쪽에 % 라벨을 그리는 커스텀 렌더러
const renderPieLabel = (props: any) => {
  const { cx, cy, midAngle, innerRadius, outerRadius, percent } = props;
  if (!percent || percent < 0.05) return null; // 5% 미만은 라벨 생략 (겹침 방지)
...
export function RmsPieChart({
  ...
  const defaultInner = donut ? '40%' : '0%';
  const defaultOuter = '75%';
  return (
    <ChartWrapper title={title} description={description} height={height} className={className}>
      <PieChart margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
        <Pie
          ...
          cx="50%"
          cy="45%"
          innerRadius={innerRadius ?? defaultInner}
          outerRadius={outerRadius ?? defaultOuter}
          dataKey="value"
          stroke="none"
          labelLine={false}
          label={renderPieLabel} // 링 안에 % 만 표시
        >
          {data.map((entry, i) => (
